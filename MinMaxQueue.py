...
class MinMaxQueue:
	def __init__(self,min_or_max):
		self.agg_func = min_or_max
		self.inStack = MinMaxStack(self.agg_func)
		self.outStack = MinMaxStack(self.agg_func)
	def push(self,value):
	...
	def query(self):
		inS = self.inStack.query()
		outS = self.outStack.query()
		if inS is None:
			if outS is not None:
				return outS
			return None
			...
			return inS
		return self.agg_func(inS,outS)
	def isEmpty(self):
		return self.inStack.isEmpty() and self.outStack.isEmpty()
	def getSize(self):
		return self.inStack.getSize() + self.outStack.getSize()
